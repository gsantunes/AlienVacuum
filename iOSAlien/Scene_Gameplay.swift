...
//  iOSAlien
//
//  Created by Guilherme on 08/08/16.
//  Copyright (c) 2016 Guilherme. All rights reserved.
//

import SpriteKit

class Scene_Gameplay: SKScene {
    ...
    func moveHero(movX: Int, movY: Int)
    {
    ...
    func moveAlien(movOX: Int, movOY: Int, movX: Int, movY: Int)
    {
    ...
    func pullAlienEgg(posx:Int,posy:Int)
    {
            if(player.mapX == posx+1 && player.mapY == posy)
            {
                swapEgg(posy,posy:posx)
            }
            else
            if(player.mapX == posx-1 && player.mapY == posy)
            {
            ...
            if(player.mapX == posx && player.mapY == posy+1)
            {
                    swapEgg(posy,posy:posx)
            }
            else
    ...
    func swapEgg(posx:Int,posy:Int)
    {
        if let alien = entityMat[posx][posy] as? Entity_AlienEgg
        {
            entityMat[player.mapY][player.mapX] = entityMat[posx][posy]
            entityMat[posx][posy] = SKSpriteNode()
            let point = CGPointMake(alien.position.x, alien.position.y)
            alien.position = player.position//CGPointMake(CGFloat(player.position.x), CGFloat(player.position.y))
            alien.mapX = player.mapX
            alien.mapY = player.mapY
            map.matrix[posx][posy] = 0
            map.matrix[player.mapY][player.mapX] = 4
    ...
    func destroyEggAt(posX:Int,posY:Int)
    {
        if let alien = entityMat[posY][posX] as? Entity_AlienEgg
        {
    ...
    func usePowerUp(){
        //percorrer matriz e deletar aliens e ovos
        //após isso atualizar a contagem de power ups dessa forma:
    ...
    override func touchesBegan(touches: Set<UITouch>, withEvent event: UIEvent?) {
        ...
        for touch: AnyObject in touches {
            ...
            var posx = (Int(touch.locationInNode(self).x ) - (originX-(tileSize/2))) / tileSize
            var posy = (Int(touch.locationInNode(self).y ) - (originY+(tileSize/2))) / tileSize
            
            if(Int(touch.locationInNode(self).x) < originX)
            {
                posx = 0
            }   
            
            if(Int(touch.locationInNode(self).y) >= originY)
            {
                posy = 0
            }
            
            if(Int(touch.locationInNode(self).x) > (tileSize * map.rowSize)+originX)
            {
                posx = map.rowSize-1;
            }
            
            if(Int(touch.locationInNode(self).y) < originY-(tileSize * map.columnSize-1))
            {
                posy = map.columnSize-1;
            }
            
            
            if(posy > 0 || posy == -map.columnSize)
            {
                posy = -(map.columnSize - 1)
            }
            ...
            if let alien = entityMat[-posy][posx] as? Entity_AlienEgg
            {
                pullAlienEgg(alien.mapX, posy: alien.mapY)
            }
        }
